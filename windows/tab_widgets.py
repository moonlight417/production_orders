...
class MainTabWidget(QWidget):
    ...
    def add_scroll_area_to_tab(self, tab, tab_name):
        ...
        # Проверка имени вкладки перед добавлением
        if tab_name == "Сборочная единица":
    ...
    def update_scroll_areas(self):
        ...
        # Получаем актуальный список названий вкладок "Сборочная единица"
        assembly_unit_tabs = [
        ...
        for tab, scroll_layout in self.scroll_areas.items():
            # Очищаем текущий layout, чтобы избежать дублирования
            for i in reversed(range(scroll_layout.count())):
                item = scroll_layout.itemAt(i)
                if item and item.widget():
                    item.widget().setParent(None)

            # Добавляем актуальный список названий вкладок «Сборочная единица»
            for name in assembly_unit_tabs:
                label = QLabel(name)
                scroll_layout.addWidget(label)

        print("Области прокрутки успешно обновлены.")

...
# from windows.order_dev_main import OrderDev
...
class MainWindowEngineer(QtWidgets.QMainWindow):
    ...
    def open_new_tasks(self):
        ...
        if self.new_tasks > 0:
            ...
            self.new_tasks_list_window.load_new_tasks()
            self.decrement_new_tasks_count()
            self.update_new_tasks_count()

    def decrement_new_tasks_count(self):
        """Уменьшение количества новых заданий на сервере"""
        task_id = self.new_tasks_list_window.get_selected_task_id()
        if task_id:
            base_url = 'http://127.0.0.1:8000/'
            api_path = f'orders/start_task/{task_id}/'
            full_url = f'{base_url}{api_path}'
            response = requests.post(full_url)
            if response.status_code == 200:
                self.new_tasks -= 1
                self.update_order_count_label()
                self.ui.BtnCheckOrders.setEnabled(self.new_tasks != 0)
            else:

...
from products.models import Drawing, MainDocument, DrawingSheet
...
class MainTabWidget(QWidget):
    def __init__(self):
        ...
        button_layout.addWidget(save_button)  # Добавляем кнопку "Обновить название"
    ...
    def save_to_db(self):
        ...
        try:
            ...
            for tag_name in tag_names:  # Проходим по всем введённым тегам
                tag, created = Tag.objects.get_or_create(name=tag_name)  # Получаем или создаём тег
                main_doc.tags.add(tag)  # Добавляем тег в основной документ

            # 3. Создание чертежа, связанного с основным документом
            drawing = Drawing.objects.create(
                main_document=main_doc,  # Ссылка на основной документ
                doc_name=self.doc_name,
                mass=self.mass,
                assembly_unit=is_assembly_unit,
            )

            # 4. Создание листа чертежа, связанного с чертежом
            drawing_sheet = DrawingSheet.objects.create(
                drawing=drawing,  # Ссылка на чертеж
                file=self.file,
            ...
            # Ошибка при сохранении данных в базе (например, уникальные поля нарушены)
    ...
    #     self.main_name = "Основной документ 1"
    #     self.comment = "Комментарий к основному документу"
    #     self.tags = ["Тег 1", "Тег 2"]  # Список тегов
    #     self.doc_name = "Чертеж 1"
    #     self.mass = 100.5
    #     is_assembly_unit = self.linked_tab.is_assembly_unit
    #     self.sheet_number = 1
    ...
    #
    #     # # Получаем текстовые значения тегов из tag_list
    #     # tag_names = [tag_input.text() for tag_input in self.tag_list if
    #     #              tag_input.text()]  # Берём только те теги, которые не пустые
    #     # print(tag_names)
    #
    #
    #     # # Получаем текстовые значения тегов из tag_list
    ...
    # def save_to_db(self):
    #     """Сохраняет данные в базу данных, если все поля заполнены."""
    #
    #     # Проверка, что все обязательные поля заполнены
    #     if not self.main_name:
    #         QMessageBox.warning(self, "Ошибка", "Пожалуйста, заполните имя задания.")
    #         return
    #
    #     # Пример проверки других полей
    #     if not self.linked_tab:
    #         QMessageBox.warning(self, "Ошибка", "Пожалуйста, выберите вкладку для задания.")
    #         return
    #
    #     # Получаем данные для сохранения
    #     is_assembly_unit = self.linked_tab.is_assembly_unit
    #     doc_name = self.main_name
    #
    #     # Создаем или обновляем задачу в базе данных
    #     try:
    #         design_doc = Drawing.objects.create(name=doc_name, is_assembly_unit=is_assembly_unit)
    #         QMessageBox.information(self, "Успех", "Документ успешно сохранён.")
    #         print(f"Задание '{doc_name}' с is_assembly_unit={is_assembly_unit} сохранено в базе данных.")
    #     except Exception as e:
